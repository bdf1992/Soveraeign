...
function buildPrompt(op) {
  return 'You are the console domain builder for Soveraeign at ' + ROOT + '. Read ' + ROOT + '/AGENTS.md and ' + ROOT + '/STATUS.yaml first. ' +
    ...
    'Hard limits: never run git commit or git push; never write console runtime code before its defeating fixtures exist; never witness or accept your own work; never touch Asset Service or Proofing Service state or lineage/evidence/. ' +
...
const plan = await agent(
  ...
  'The founding docket is closed: open_decisions is empty and the O<n> identifiers are retired (decisions/0033-close-the-founding-docket.md). Settle a decision at the lowest tier that can produce evidence defeating the alternatives, and record what would defeat the ruling. Only PUBLIC-CLEARANCE and owner-held product intent, public naming, external commitment, irreversible external effect, secrets, and destructive repository administration reach Bdo, and his gate is acceptance over an evidenced result, never permission to begin. Current standing: console_service_status is BUILT_CONTINUITY_PATH_SELF_TESTED_REMAINDER_BOUNDARY. The continuity path is built and self-tested; the boundary, the name Console, and the provisional Human Binding target are accepted (decisions/0033-close-the-founding-docket.md). What the remaining operator surfaces are missing is the contract and its defeating fixtures, not permission. The ruling is defeated by a Console record producible only through the human interface, which would prove the interface is authority rather than a binding. ' +
