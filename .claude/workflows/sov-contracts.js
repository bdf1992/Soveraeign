...
const scopePrompt = 'Read ' + ROOT + '/AGENTS.md and ' + ROOT + '/STATUS.yaml fully, then ' + ROOT + '/contracts/README.md, the six schema files in ' + ROOT + '/contracts/, the logical object blocks in ' + ROOT + '/SPEC.md, and the vocabulary tables in ' + ROOT + '/CLASSIFICATION.md. Produce a bounded operation plan for: ' + objective + '. Honor the gates recorded in STATUS.yaml: O10 (gates spec.ratify) means every schema change is marked a proposal and carries a positive and a defeating conformance case; O9 gates classification.ratify; O12 gates model_binding.ratify_contract; O4 gates attestation.ratify_schema. Every operation declares effect_class RECORD_LOCAL or RESOURCE_CONSUMPTION; EXTERNAL_WORLD is refused in Phase I. Never decide judgement-typed questions; put each in judgement_queue. Blocked edge is not blocked frontier (AGENTS.md, Self-direction is not delegation): an unresolved owner decision gates only the transition that needs it. Plan the reachable precursors, take reversible defaults for every other choice and name them in the operation descriptions, and set blocked true only when no admissible operation exists for this objective. Each judgement_queue entry names the transition it gates.'
