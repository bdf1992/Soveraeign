...
- `python scripts/verify.py` runs 41 checks and grades itself on wall
  ...
  `python scripts/lint.py` passes with no named module debt. The duplicate
  `STATUS.yaml` keys it used to carry are deleted, and a repeated key in a YAML
  document is now a defect rather than a warning: a reader keeps the last value
  and drops the rest in silence, so the earlier line reads as current to a
  person and means nothing to a program.
