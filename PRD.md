...
Delivery Substrate**. Whether this phase is open is `STATUS.yaml` and
`contracts/phases.json`'s reading to give (`python scripts/sov_next.py`), not this
profile's. `STATUS.yaml` remains the authority for current phase standing, and
merely writing, testing, or agreeing with this profile earns no exit clause
standing.
...
- **`phase:i` closed incomplete**, with five residual custodies `PROPOSED`.
  `contracts/phases.json` carries its `succeeded_by` reading; which roadmap phase
  opens next is Bdo's to set.
