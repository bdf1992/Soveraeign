...
qualification profile in `PRD.md`. They carry no phase standing on their own;
`STATUS.yaml` and `contracts/phases.json` are the sole record of whether that
phase is open. Controller, Orchestrator, Worker, and Witness are one proving
arrangement of these primitives, not privileged Kernel roles.
