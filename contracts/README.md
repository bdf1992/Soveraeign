...
| Contract | Purpose |
| --- | --- |
| `service-manifest.schema.json` | declares a service boundary, ownership, dependencies, operations, Kernel contracts used, and forbidden shortcuts; decisions/0062 proposes reading each manifest as that service's authored Kernel Binding |
...
| `work-lease.schema.json` | PROPOSED (decisions/0056): one concern actively possessed by one attributable machine principal, under a grant, inside a multidimensional budget, against a stated closure condition; it grants nothing |
...
| `kernel-paradigms.schema.json` | PROPOSED (decisions/0062): shape of the registry that resolves stable service-manifest Kernel contract ids to the governing sources defining each paradigm |
| `kernel-paradigms.json` | PROPOSED (decisions/0062): addressable index of the current standing, typed-authority, operation, observation, receipt, retraction, and attestation Kernel paradigms; the indexed sources remain authoritative |
| `kernel-closure.schema.json` | PROPOSED (decisions/0062): shape of the rebuildable Node-wide projection showing service bindings, paradigm participation, type ownership, transition usage, and explicitly unmapped operations |
