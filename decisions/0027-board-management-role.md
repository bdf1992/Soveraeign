...
`LABEL_ADD`, `LABEL_REMOVE`, `BRANCH_DELETE`. Everything else is `REPORT`:
`CONTRACT_DEFECT`, `CONTRACT_BEHIND`, `LABEL_UNMAPPED`, `PR_STALE`. A report
appears on the same surface, is rendered `OBSERVE_ONLY`, and is refused by name
if an approval names it. Repairing a metadata block needs authorship and closing
...
**One write crossing, admitting three verbs.** `adapters/github/apply.py` is the
only module permitted to write to GitHub, as `export.py` is the only one
permitted to read. It admits exactly the three proposable kinds and refuses any
...
- The board becomes maintainable in one pass instead of accumulating. The first
  live survey found 23 missing labels across seven issues, 11 merged branches
  whose refs still existed, three pull requests quiet for over twelve hours, and
  two issues invisible to the work queue for want of a valid metadata block.
...
- The registrar's declared input projection widens: pull requests now carry
  `isDraft` and `updatedAt`, and branch names are captured. Both are read-only
  additions to an existing crossing and are recorded in
...
- `conformance/fixtures/board/survey-cases.json`: 14 cases, 7 positive and 7
  defeating, run offline by `python scripts/sov_board.py selfcheck` and wired
  into `scripts/verify.py`.
- `scripts/tests/test_sov_board.py`: 25 unit tests over action dispositions,
