...
1. **Self-direction is not delegation.** A participant exercises judgement
   ...
   choice binding on others. It can construct what it cannot accept, test
...
"inspect, compare, draft, implement, test, and machine-accept". The defect was
...
- `AGENTS.md` Authority (pre-change): agents "may inspect, compare, draft,
  implement, test, and machine-accept"
...
| O2 | `engineering.accept_baseline` |
| O3 | `attestation.bootstrap_first_attestor` |
| O4 | `attestation.accept_schema` |
| O5 | `gauge.accept_specification` |
| O6 | `kernel.make_effective_unattestable` |
| O7 | `kernel.commit_external_world_effect` |
| O8 | `qualification.settle_cold_start` |
| O9 | `classification.accept` |
| O10 | `spec.accept`, `roadmap.close_f1` |
| O11 | `proofing.accept_boundary` |
| O12 | `model_binding.accept_contract` |
| O13 | `sdlc.activate_loop` |
| O16 | `coordination.activate_external_effects` |
| O17 | `sov.activate_live` |
| O18 | `console.accept_boundary`, `console.authorize_provisional_human_binding` |
| O19 | `ticket_kind.accept_verification_engagement` |
| O20 | `channels.activate_projection` |
| O21 | `projection.accept_boundary` |
| O22 | `ticket_kind.accept_story`, `classification.accept` |
...
- Mapped O2 (`production_implementation`) to acceptance of the baseline
...
- Mapped the three `<service>_implementation` labels to
  `<service>.accept_boundary`; implementation stays behind the fixture
...
- Left `scripts/sov_epic.py` `HELD ... blocked by` untouched: a ticket held
  ...
  word.
- Recorded O22's gate as both `ticket_kind.accept_story` and
  `classification.accept`, since its question bundles a vocabulary entry.
