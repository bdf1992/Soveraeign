# 0065 · The standing landing grant is accepted

Status: `OWNER-DIRECTED · ACCEPTED POLICY`

## Decision

`grant:standing-landing-loop` moves from `PROPOSED` to `ACCEPTED` in
`contracts/standing-grants.json`. Bdo accepted it on 2026-08-25, in session, having been
...
it forbids *the loop* from accepting the grant the loop is about to spend, because a
...
`VERIFICATION` type cannot accept a `JUDGEMENT` claim (`SPEC.md`, PROD-I-5), so it could
...
the artifact lifecycle reserves, and it is not `ACCEPTED`. Landing removed a conversation
...
- The grant is accepted as shipped rather than narrowed. Dropping `docs/` from scope was
