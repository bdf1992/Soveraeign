...
Candidate `03fd200` on `claude/witness-context-handoff-n8lk0h`, tree read from that
...
working tree that built it. Sixteen readings have been commissioned.
...
## What sixteen readings did

The concern's own repair — the witness frame — was straightforward. The **guard** was not.
Sixteen independent readings defeated it seventy-five times, and each found the same failure one
layer further out:
...
The repair is that a file declaring a frame must deliver it **exactly**, not merely open on
it. Each dispatch's own assignment became the frame's last block, so there is no longer a
tail to append to. Two more from the same reading: a template literal collapsed to one
opaque expression, which delivered the package's own inversion sentence in text neither
grader could see — `render.py` now refuses a construction it cannot render, which is what
its own docstring always claimed; and a conditional element inside the frame rendered as an
opaque block between two graded ones, so every frame block must now open on a literal head.

`scripts/tests/fixtures/witness-context-defeats.json` carries all seventy-five constructions
...
on any refusal no case fires, on any register entry naming a refusal that no longer exists,
and on any entry a case now fires.
