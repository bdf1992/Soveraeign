...
**Five readings, one failure, five places.** Every one was a question this package answered
twice: where a group ends, how many values a run of expressions carries, which bracket
closes which, what a frame's separator is. Aligning two readers is not a repair, because
there is always another place they have not been compared. So the questions are asked once
now. `scan.py` holds the mask, the statement reader and the single balancer every other
module calls. `render.read` is one walk returning both what a prompt delivers and where each
spliced value sits, and `dispatch.expression_spans` is half of that answer rather than a
second traversal. The frame's separator is read from its own `.join(...)` and must be
whitespace, because it is delivered between every pair of blocks and belongs to none of them.

The same reading found a prompt built by a static method of a class — neither followed nor
refused, because a class is not a binding the reader knew. It is refused now. It also
re-measured the position rule's reach and got a different number from mine; the figure this
report now states, eight of one hundred splices across twenty-eight non-frame dispatches, is
measured on the current tree.

`scripts/tests/fixtures/witness-context-defeats.json` carries all ninety-three constructions
with contiguous ids, and every refusal the package states is fired by a case or recorded
with the gate that pre-empts it.
