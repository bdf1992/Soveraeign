...
Candidate `bb496a0` on `claude/witness-context-handoff-n8lk0h`, tree read from that
...
working tree that built it. Fourteen readings have been commissioned.
...
## What fourteen readings did

The concern's own repair — the witness frame — was straightforward. The **guard** was not.
Fourteen independent readings defeated it sixty-five times, and each found the same failure one
layer further out:
...
so `witnessFrame = function (...)` placed after the declaration — or a second declaration
that hoisting prefers — left the real frame in the file as dead code for every rule to
grade while the evaluator received another. `verify.py` stayed at exit 0 with the subject
pin gone and the build report named the oracle. The package already refuses a twice-written
binding in two other places; the rule had simply never been pointed at the frame. It is now.

`scripts/tests/fixtures/witness-context-defeats.json` carries all sixty-five constructions
with contiguous ids. Which refusals are demonstrated is no longer prose: a case reads every
refusal message out of the package's own syntax tree, runs the corpus, and fails the build
on any refusal no case fires — unless a register names the earlier gate that makes it
unreachable, with its reason. The fourteenth reading counted eleven undemonstrated refusals
and could not tell those two apart; that is what the register records.
