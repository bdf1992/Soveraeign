...
Candidate `293a4cc` on `claude/witness-context-handoff-n8lk0h`, tree read from that
commit, base `64fe985`, forty paths, every one inside `grant:standing-landing-loop`.
`verify` PASS and `lint` PASS measured at that commit in a clean worktree, not in the
working tree that built it. Thirteen readings have been commissioned.
...
## What thirteen readings did

The concern's own repair — the witness frame — was straightforward. The **guard** was not.
Thirteen independent readings defeated it sixty times, and each found the same failure one
layer further out:
...
`scripts/tests/fixtures/witness-context-defeats.json` carries all sixty constructions with
contiguous ids, and every refusal the module states is fired by at least one of them —
...
- **A guard is only ever as good as its last reading.** Thirteen readings produced
  thirteen dissents. Each repair was at the cause and each was pinned by a case, and the
  thirteenth still found three. The honest claim is not that this package cannot be
  defeated; it is that sixty named constructions cannot defeat it, and that the corpus
  makes the next defeat cheap to add.
- **Presence is graded, meaning is not, and only the frame escapes that.** Twenty-two of
  the twenty-three workflows are held to a sentence plus the position of what they splice.
  Two independent readings showed what that cannot catch: a prompt may open on the
  demotion and then, in its own prose, tell the evaluator to take the builder's scope as
  given and not to pin a subject. No presence or position rule reaches it. Identity does —
  the frame's rendering compared byte for byte against what the dispatch delivers — and
  `sov-loop.js` is now held to that. Extending it to the other twenty-two means each
