...
Candidate `aab9b54` on `claude/witness-context-handoff-n8lk0h`, tree read from that
...
working tree that built it. Seventeen readings have been commissioned.
...
## What seventeen readings did

The concern's own repair — the witness frame — was straightforward. The **guard** was not.
Seventeen independent readings defeated it seventy-eight times, and each found the same failure one
layer further out:
...
It also measured what the weaker rule actually does: across the twenty-one non-frame
workflows, twenty-eight dispatches and eighty-nine splices, the position half grades
**three**. For the rest the demotion sits early and the effective rule is presence alone.
That is now stated as what it is rather than as more.

`scripts/tests/fixtures/witness-context-defeats.json` carries all seventy-eight
constructions with contiguous ids, and every refusal the package states is fired by a case
or recorded with the gate that pre-empts it — a register that now fails the build three
ways, including when a case starts firing an entry it exempts.
