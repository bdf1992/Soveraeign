...
Candidate `f7cd21e` on `claude/witness-context-handoff-n8lk0h`, tree read from that
...
working tree that built it. Nineteen readings have been commissioned.
...
## What nineteen readings did

The concern's own repair — the witness frame — was straightforward. The **guard** was not.
Nineteen independent readings defeated it eighty-nine times, and each found the same failure one
layer further out:
...
| 19 | the same collapse a fourth time, between two adjacent splices |
...
group *inside* a frame block was consumed whole and emitted as a single marker, so a
countermand and the builder's account were delivered and present in nothing any grader read
— and because both the frame's rendering and the delivered text collapsed the same region
the same way, identity matched *because neither side could see it*.

The nineteenth found it a fourth time and named the cause. `_literals` emitted one marker
per *run* of expressions while `interpolated` reported each one, so `declared +
built.summary` inside a frame block read as one value where two were delivered, and `ROOT +
claims` put an agent result beside the single expression the position rule inspects. The
whole mutation was deleting a string literal between two splices. It also measured that the
frame's own declared budget was already wrong on the pristine tree, and that three of five
register entries named a gate that does not fire, because `_balanced` counted parentheses
in raw source while every other balance reader used the mask.

The twentieth found it a fifth time, inside the pair the nineteenth's repair had made "the
same walk". `_literals` balanced a group on the mask; `expression_spans` balanced the same
group on raw text. One handled a regex and refused a template literal; the other did
neither. An unmatched `)` inside a string literal — `('PRIOR REPORT, authoritative :) ' +
JSON.stringify(buildResults) + ' ')` — desynchronised them, and the account reached the
evaluator at character 31 while the rule inspected a slice that did not contain it.

The twenty-first found it a sixth time. `render.read` still carried a private depth scan
over raw source; `_own_returns` was a second private brace balancer whose pattern matched an
expression-bodied arrow and then blanked the *next* unrelated block; `calls._object_member`
was a third scan over raw text. A `(` inside a string literal — `KNOWN.join(' (')` —
unbalanced the first, swallowing every literal after it into one value, so a countermand
naming the account the oracle was delivered and read by nothing. `const tidy = x => x` above
the frame hid one of two returns from the second.

**Six readings, one failure, six places.** Every one was a question this package answered
twice: where a group ends, how many values a run of expressions carries, which bracket
closes which, what a frame's separator is, where a nested function body begins. Twice the
divergence was inside the pair a previous repair had just unified. Aligning two readers is
not a repair — there is always another place they have not been compared.

So the questions are answered once. `scan.py` holds the mask, the statement reader and the
one balancer every module calls. `render.read` is a single walk returning both the delivered
text and the span of each spliced value; `dispatch.expression_spans` returns half of that
answer rather than walking again; a frame's blocks are rendered through the same resolution
its dispatch's own text goes through. Indirection is followed to a fixpoint or refused — a
helper, an object member, a class method, a getter, a computed member with a literal key —
and a call through a locally bound name that is neither a JavaScript method nor one this
reader can follow is refused rather than rendered as an opaque value.

`scripts/tests/fixtures/witness-context-defeats.json` carries all hundred and one
constructions with contiguous ids, and every refusal the package states is fired by a case
or recorded with the gate that pre-empts it.
