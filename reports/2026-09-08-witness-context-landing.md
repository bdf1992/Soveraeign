...
Four readings, one failure, four places: a region two readers counted differently. What
closes the class is not another reader but that the two walks are now the same walk — both
descend into a group, both emit one marker per value — and that the frame's shape is
written down rather than inferred: `FRAME_SPLICES` records what each block hands over, no
head may repeat, and no head the rules do not know is admitted.

The nineteenth also found two questions only `test_witness_context_provenance` asked — a
doubled concatenation operator, and a file naming the witness agent that yields no dispatch.
The corpus grades only `audit.violations`, so no case could ever have reached them. Both
are asked by that predicate now.

`scripts/tests/fixtures/witness-context-defeats.json` carries all eighty-nine constructions
