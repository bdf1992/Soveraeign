...
Sixty-four `sov_*.py` entrypoints plus the two verification commands. This is the
...
| `sov_comms.py` | `grade` `check` `selfcheck` | Whether what the Communications seat says is supported: a figure with nothing that derives it, machine vocabulary in a message to a person, a witnessed claim no witness record carries. |
