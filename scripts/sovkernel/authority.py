...
def independence_basis(request: dict) -> str:
    ...
    What it still does not do is join the observer against the session registry's path
    claims. That store is per-machine and gitignored, so a check reading it would pass
    wherever it is absent. Stated here rather than implied.
    """
