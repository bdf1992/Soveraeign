...
def independence_basis(request: dict) -> str:
    ...
    An independent reading then defeated that form too: it returned MEASURED for any
    non-empty `observation_path` string, without ever comparing it to anything. A path
    that is merely present is still a declaration. MEASURED now requires both sides the
    comparison needs - the observation's path and the paths being landed - so the word
    names a comparison that ran rather than a field that was filled in.

    Two limits, both stated because a reading found each one overstated. This module
    reads no files, so that `observation_path` names a real file is the caller's
    invariant; `sov_land.py` and `sovland/candidates.py` each set it only after reading
    the observation from that path. And it does not join the observer against the session
    registry, which is per-machine and gitignored, so a check reading it would pass
    wherever it is absent. What is measured is narrow: an observation was supplied, from a
    path the caller resolved, against a change with paths to compare it to.
    """
