...
def independence_basis(request: dict) -> str:
    ...
    An earlier form returned MEASURED when the observer's own id appeared inside one of
    the landed paths: entanglement read as evidence of independence, and unreachable by
    any observer not named after a file.

    Two readings then defeated that form in turn: it returned MEASURED for any non-empty
    ...
    It still does not join the observer against the session registry, which is per-machine
    and gitignored, so a check reading it would pass wherever it is absent. What is
    measured is narrow: an observation was supplied, from a path the caller resolved,
    against a change with paths to compare it to.
    """
