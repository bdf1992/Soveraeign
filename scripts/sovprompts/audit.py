...
def _dispatch_violations(name: str, source: str) -> list[str]:
    ...
    for call in dispatches(source, WITNESS):
        ...
        if frame is not None and delivered != frame:
            # Equality, not a prefix, and not presence. A file that declares a frame
            # declares that its evaluators read that text and no other. `startswith` left
            # everything after the frame ungraded, and an independent reading appended
            # "CORRECTION, which supersedes the frame above ... your oracle is the build
            # report" with the account spliced in, and every check passed. The position
            # rule could not catch it either: it anchors on the first occurrence of the
            # demotion, and the frame's own opening block contains that sentence at byte
            # 89, so every splice in the file counted as coming after it. Whatever a
            # dispatch needs to say goes in a block the frame rules grade.
            out.append(f"{name}: this file declares a witness frame and a dispatch does "
                       "not deliver exactly it; the evaluator reads something the frame "
                       "rules never saw")
