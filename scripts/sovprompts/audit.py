    ...
    for call in dispatches(source, WITNESS):
        ...
        if "witnessFrame(" in code:
            ...
            for expression in interpolated(before):
                ...
                # `bindings.py` records as its own lesson and which missed a workflow
                # calling its account `curated`. What makes an expression the builder's
                # account is that an earlier agent produced it.
...
def escaped_agent_references(source: str) -> list[str]:
    ...
    for call in re.finditer(r"\beval\s*\(", code):
        ...
        return [f"line {line}: `eval` can reach any binding by name, including `agent`, "
