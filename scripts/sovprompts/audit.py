...
#: How many values each block of the frame hands an evaluator. The frame is the one text
#: the whole claim rests on, so its shape is declared here rather than inferred: an
#: independent reading inserted a countermand carrying the builder's summary into a block
#: that already demoted what it spliced, and no presence or position rule could see the
#: difference between the value the frame means to pass and one added beside it. A splice
#: added, moved or removed anywhere in the frame changes this table, and changing the
#: frame means saying so here.
FRAME_SPLICES = {
    "SUBJECT": 0, "SCOPE": 1, "CONTRACT": 0, "CHECKS": 0, "STATE": 0,
    "THE BUILDER'S ACCOUNT": 3, "INDEPENDENCE": 1, "TERMINAL": 0, "RECORD": 4,
}
FRAME_OWNERS = {
...
def _frame_violations(name: str, source: str) -> list[str]:
    ...
    for head, block in zip(heads, parts):
        declared = next((count for name, count in FRAME_SPLICES.items()
                         if head.startswith(name)), None)
        if declared is None:
...
def _dispatch_violations(name: str, source: str) -> list[str]:
    ...
    out: list[str] = []
    try:
        frame = rendered(source, FRAME) if declares(source, FRAME) else None
    except Unreadable:
