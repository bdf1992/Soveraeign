...
from sovprompts.names import _after, agent_names as _agent_names, writes_by_name as _writes_by_name
...
def _object_member(text: str, key: str) -> str | None:
    ...
    Bracket depth on the mask, like every other boundary reader in this package. This was
    a third private scan over raw text, and a boundary answered twice is how six
    consecutive independent readings defeated this package.
    """
    code = masked(text)
    shorthand = code.lstrip().startswith("{")
    # `key: value`, `get key()` and the `key(...)` method shorthand are all one member.
    # A getter was the shape an independent reading used to deliver prose: the reader
    # looked only for a colon, found nothing, and left the reference an opaque marker.
    # The shorthand forms count only inside an object literal. Read anywhere, `map(` in
    # `doms.map(...)` is a call on a value, not a member the receiver declares.
    shape = rf"\b(?:get\s+|set\s+)?{re.escape(key)}\s*(?::|\()" if shorthand \
        else rf"\b{re.escape(key)}\s*:"
    match = re.search(shape, code)
    if not match:
        return None
    # Whitespace skipped in the source, not on the mask: a blanked string literal is a run
    # of spaces there, so a trailing `\s*` in the pattern would run straight past the value.
    begin = _after(text, match.end() - 1 if code[match.end() - 1] == "(" else match.end())
    index, depth = begin, 0
