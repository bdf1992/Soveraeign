...
from sovprompts.scan import _inside, _string_spans
...
def _agent_names(source: str) -> set[str]:
    ...
    A bespoke one-hop regex closed that and left four more open: a two-hop alias, a bare
    reassignment, a member call, and a destructured alias. `written_into` already resolves
    all four and was already imported here, so the special case is gone and the fixpoint
    below uses the machinery every other binding goes through. A call that is never
    recognised produces no refusal, so failing closed cannot reach it - which is why this
    has to be resolution rather than a pattern.
    """
    names = {"agent"}
    candidates = set(re.findall(r"\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)", source))
    candidates |= set(re.findall(r"\b([A-Za-z_$][\w$]*)\s*=(?!=)", source))
    candidates |= set(re.findall(
        r"\b[A-Za-z_$][\w$]*\s*\.\s*([A-Za-z_$][\w$]*)\s*=\s*agent\b", source))
    # Destructured names are bound without ever appearing before `=`, so neither pattern
    # above sees them and `const { x } = { x: agent }` stayed unenumerated.
    for group in re.findall(r"\b(?:const|let|var)\s*[{\[]([^}\]]*)[}\]]\s*=", source):
        candidates |= {piece.strip().split(":")[-1].strip()
                       for piece in group.split(",") if piece.strip()}
    candidates = {name for name in candidates if IDENT.fullmatch(name or "")}
    changed = True
    while changed:
        changed = False
        for name in sorted(candidates - names):
            for written in written_into(source, name):
                # A destructured binding takes one member of what it is written from, so
                # `const { x } = { x: agent }` writes the whole literal and binds only
                # `x`. Comparing the whole write alone left that alias unenumerated.
                candidate = written.strip().rstrip(";").strip()
                member = _object_member(written, name)
                if candidate in names or (member or "").strip() in names:
                    names.add(name)
                    changed = True
                    break
    return names
...
def agent_calls(source: str, permissive: bool = False) -> tuple[tuple[str, str | None], ...]:
    ...
    spans = _string_spans(source, permissive)
