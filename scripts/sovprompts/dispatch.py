...
def _member_source(source: str, root: str, text: str) -> str | None:
    ...
    if re.search(rf"\bclass\s+{re.escape(root)}\b", masked(source)):
        # A class this file declares, whose method this reader does not follow. It found
        # nothing in `written_into` - a class is not a binding it reads - so the call
        # stayed an opaque expression, neither followed nor refused, and an independent
        # reading put the builder's account inside a static method exactly there.
        raise Unreadable(
