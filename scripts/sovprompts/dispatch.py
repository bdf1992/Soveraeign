...
def expression_spans(prompt: str) -> list[tuple[int, int]]:
    ...
    while index < length:
        ...
        if char.isspace() or char == "+":
            ...
            continue
        if char == "(":
            # Inside the group, not past it, because `render._literals` reads inside it
            # too. One marker per expression on one side and one per group on the other
            # was the same two-readers-one-question divergence four readings have now
            # found in different places; the position rule correlates the nth marker with
            # the nth expression, so the two walks have to be the same walk.
            close, depth = index, 0
            while close < length:
                if prompt[close] == "(":
                    depth += 1
                    ...
                    if depth == 0:
                        break
                close += 1
            inner = expression_spans(prompt[index + 1:close])
            out.extend((index + 1 + a, index + 1 + b) for a, b in inner)
            index = close + 1
            continue
        start, depth = index, 0
        while index < length:
