...
def blocks(source: str, name: str, resolve=None) -> list[str]:
    ...
    # Through the same resolution a dispatch's own text goes through, so the frame's
    # rendering and the bytes delivered are produced by one path. Rendering the blocks
    # from source while the dispatch resolved its indirection was one more place two
    # readers answered a question differently.
    read = [_literals(resolve(source, part) if resolve else part) for part in elements]
...
def rendered(source: str, name: str, resolve=None) -> str:
    ...
    return separator.join(blocks(source, name, resolve))
