...
VALUE_ENDERS = {")", "]", "}"}
VALUE_WORDS = {"this", "true", "false", "null", "super"}
PAIRS = {")": "(", "]": "[", "}": "{"}
    ...
    while index < len(text):
        ...
        if char == "/":
            divides = last[0] in ("number", "string", "regex") or (
                last[0] == "word" and last[1] not in VALUE_WORDS - VALUE_WORDS) or (
                last[0] == "word") or last[1] in VALUE_ENDERS
            if divides and not (last[0] == "word" and last[1] in {"return", "typeof", "case"}):
                last = ("punct", "/", line)
                yield last
                index += 1
                continue
