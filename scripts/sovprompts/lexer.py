...
from sovprompts.scan import BEFORE_REGEX_WORDS  # noqa: E402
PAIRS = {")": "(", "]": "[", "}": "{"}
...
def significant(text: str) -> Iterator[tuple[str, str, int]]:
    ...
    while index < len(text):
        ...
        if char in "'\"`":
            ...
            continue
        if char == "/":
            # This read `last[1] not in VALUE_WORDS - VALUE_WORDS`, an empty set, so the
            # clause was unconditionally true and the `or (last[0] == "word")` beside it
            # made every word divide; the set it named was dead and is now gone. `else /"/.test(x)`, `in`, and `void` were each convicted as
            # unterminated strings, on JavaScript node accepts. The word list that says a
            # regex may follow now comes from `scan.py`, so this reader and the mask
            # This reader and `scan.masked` now agree on that one question. They have not
            # been unified: a ninth reading counted eight partial scanners still standing
            # across this package, and found two that genuinely disagree - `render._body`
            # truncates at a `}` inside a string, and this module treats `}` as ending a
            # value where `scan` treats it as opening a regex. Neither disagreement turns
            # into a missed refusal today, because both fail closed, and that is luck
            # rather than design. Saying they cannot drift apart was false and is withdrawn.
            divides = last[0] in ("number", "string", "regex") or (
                last[0] == "word" and last[1] not in BEFORE_REGEX_WORDS
            ) or last[1] in VALUE_ENDERS
            if divides:
                last = ("punct", "/", line)
                yield last
                index += 1
