...
def _after(source: str, index: int) -> int:
    """The first non-space offset at or after `index`, read in the source and not the mask.

    A pattern ending `=\\s*` was matched against `masked(source)`, where a string literal is
    a run of spaces, so the `\\s*` ran straight through the value being assigned and the
    statement was read from whatever followed it. An independent reading measured the
    consequence: forty-three bindings that this module and `bindings.written_into` read
    differently, including one that resolved to a schema declared several statements away.
    """
...
def writes_by_name(source: str) -> dict[str, tuple[str, ...]]:
    ...
    code = masked(source)
    for pattern in (r"\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=(?!=)",
                    r"\b([A-Za-z_$][\w$]*)\s*=(?!=)"):
        for match in re.finditer(pattern, code):
            out.setdefault(match.group(1), []).append(_statement(source, _after(source, match.end())))
    for match in re.finditer(r"\b(?:const|let|var)\s*[{\[]([^}\]]*)[}\]]\s*=(?!=)", code):
        written = _statement(source, _after(source, match.end()))
