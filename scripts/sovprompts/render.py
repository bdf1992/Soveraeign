...
from sovprompts.scan import _balanced, _opens_regex, masked, regex_end
...
def read(source: str, offset: int = 0) -> tuple[str, list[tuple[int, int]]]:
    """What a prompt expression delivers, and where each value it splices in sits.

    One walk, returning both answers. They used to be two walks in two modules kept in
    step by hand - `render._literals` and `dispatch.expression_spans` - and five
    consecutive independent readings defeated this package at a place where the two
    disagreed: a call-site argument, a parenthesised group, adjacent expressions collapsing
    to one marker, parentheses balanced on raw source in one and on the mask in the other,
    and then a regex and a template literal handled by one walk and not the other. Each
    time the fix was to make them agree again, and each time a reading found the next place
    they did not. Two readers cannot disagree about a question only one of them answers.

    `offset` is added to every span, so recursing into a group reports positions in the
    caller's own text.
    """
    out: list[str] = []
    ...
    while index < length:
        ...
        if char == "`":
            ...
            # marker, so its text reached the evaluator and appeared in nothing either
        ...
        if char == "(":
            ...
            # the rendering and the delivered text collapsed the same region the same way,
            # the identity comparison matched because neither side could see it.
            close = _balanced(masked(source), index, "(", ")")
            if close == -1:
                raise Unreadable("a group opens and never closes in a prompt expression")
            inner, inner_spans = read(source[index + 1:close], offset + index + 1)
            out.append(inner)
            spans.extend(inner_spans)
            index = close + 1
            continue
        if char == "/" and _opens_regex(source, index):
            # Ended by the mask's own rule, so nothing after a regex leaves the reading.
            start = index
            index = regex_end(source, index)
            out.append(EXPR)
            spans.append((offset + start, offset + index))
            continue
        # Anything else is a value: consume to the next top-level + or literal, counting
        # brackets on the mask. This counted them in raw source, so a `(` inside a string
        # literal - `KNOWN.join(' (')` - left the scan unbalanced and it swallowed every
        # literal after it into one value. An independent reading used exactly that to
        # deliver the package's own inversion sentence in text no rule read. It is the
        # sixth time a boundary answered twice has defeated this package, and it was in
        # the function the last repair had unified.
        code = masked(source)
        start, depth = index, 0
        while index < length:
            here = code[index]
            if here in "([{":
                ...
                depth -= 1
            elif depth == 0 and (source[index] in "'\"" or here == "+"):
                break
            index += 1
        if index == start:
            raise Unreadable(f"cannot read expression at {source[start:start + 40]!r}")
        # One marker per value, never one per run of them. This emitted one per run while
        # the other walk reported each - `declared + built.summary` read as one value where
        # two were delivered - and deleting a string literal between two splices was the
        # whole mutation an independent reading needed.
        out.append(EXPR)
        spans.append((offset + start, offset + index))
...
def _literals(source: str) -> str:
    """What a prompt expression delivers, with each spliced value marked."""
...
#: A nested function body opens after `function ...(...)` or after `=>`. Matched only when
#: a brace actually follows, because `=>\s*` alone matched an expression-bodied arrow and
#: then blanked the *next* unrelated block: an independent reading hid a decoy `return`
#: behind `const tidy = x => x` and the frame read one return where two existed.
NESTED = re.compile(r"\bfunction\b[^(){}]*\([^()]*\)\s*\{|=>\s*\{")
...
def _own_returns(code: str) -> list[int]:
    ...
    an inline callback read as branch-dependent, and counting too few let a guarded decoy
    return sit above the real one, which is the defeat this whole check exists to refuse.
    Length is preserved so offsets stay valid in the caller's own text.
    """
    out = list(code)
    for match in NESTED.finditer(code):
        brace = code.rindex("{", match.start(), match.end())
        close = _balanced(code, brace, "{", "}")
        for position in range(brace, (close + 1) if close != -1 else len(code)):
            out[position] = " "
