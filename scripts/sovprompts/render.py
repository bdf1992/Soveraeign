...
def _literals(source: str) -> str:
    ...
    while index < length:
        ...
        if char == "(":
            ...
            out.append(_literals(source[index + 1:close]))
            index = close + 1
            continue
        ...
        # One marker per expression, never one per run of them. This collapsed adjacent
        # expressions into a single marker while `dispatch.interpolated` reported each
        # separately - two readers, one question, different answers - and an independent
        # reading used the gap four ways at once: `declared + built.summary` inside a frame
        # block read as one value where two were delivered, and `ROOT + claims` put an
        # agent result beside the one expression the position rule inspects. Deleting a
        # string literal between two splices was the whole mutation.
        out.append(EXPR)
    return "".join(out)
