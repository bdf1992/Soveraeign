...
from sovverify.shape import DEV_GATE, ROOT, Check
...
REPOSITORY_CHECKS = (
    ...
    Check("orientation snapshot", [sys.executable, "scripts/sov_snapshot.py", "check"],
          ROOT,
          "re-derives every number at the moment of the check - git ls-tree for the counted "
          "directories and git rev-list for the history, both of the commit at HEAD, and the "
          "working tree for the two counts the repository already computes - and never reads the "
          "page's own claim about being current; the page is orientation for every launched "
          "agent, which does not carry the interactive session's context to correct it "
          "(LESSONS.md L-0001)",
    ...
    Check("recorded traps still hold", [sys.executable, "scripts/sov_traps.py"], ROOT,
          "re-derives every recorded trap from the repository at check time, so a trap that has "
          "stopped being true fails here instead of going stale in prose",
    ...
    Check("status claims are typed",
          ...
          "duplicated keys, and grades it against a crosswalk holding no copy of it; an untyped "
          "field and a stale entry both fail",
    ...
    Check("owner queue", [sys.executable, "scripts/sov_accept.py", "audit"], ROOT,
          "fails when anything sits on the owner without a complete packet, reading the declared "
          "acceptance contract rather than any claim that a result is ready",
    ...
    Check("acceptance routing", [sys.executable, "scripts/sov_docket.py", "check"], ROOT,
          "reads the decision records and the two declared contracts and proves the crosswalk is "
          "total, no routing names a record that does not exist, and every claim that "
    ...
    Check("signpost reconciliation", [sys.executable, "scripts/sov_next.py", "--strict"], ROOT,
          "re-reads STATUS.yaml, ROADMAP.md and the epic projection at the moment of the check; "
          "it does not consult any prior reconciliation",
          ("ROADMAP.md", "STATUS.yaml", ".claude/epic/tree.json")),
    Check("diagram provenance", [sys.executable, "scripts/sov_diagrams.py"], ROOT,
          "recomputes each declared source digest from the file's bytes at the moment of the "
          "check; it never reads a diagram's own claim about being current",
          ("diagrams",)),
    Check("witness receipts against the tree",
          [sys.executable, "scripts/sov_witness_layer.py", "records"], ROOT,
          "recomputes every digest a witness receipt declares from the subject's bytes at the "
          "moment of the check; it reads no field in which a receipt states its own freshness "
          "and never asks a subject whether it changed. Subject drift is reported as debt "
          "because a receipt observes a named commit and never claimed to describe the present; "
          "a receipt that digests nothing, or whose own probe moved, fails",
    ...
    Check("witness probes still reach",
          ...
          "parses each probe and requires the repository paths its own source declares as its "
          "reach to exist and to be used, so a probe aimed at a deleted subject fails here "
          "rather than going on producing receipts; it grades no check the probe makes, because "
          "a probe observes and never settles",
    ...
    Check("conformance oracle tests",
          ...
          "tests the oracle from outside itself, including cases proving it refuses reports it "
          "cannot read",
          ("conformance/tests",)),
    ...
    Check("kernel transition contract", [sys.executable, "scripts/sov_kernel.py", "selfcheck"],
          ROOT,
          "judges a declared corpus of positive and defeating requests against the authored "
          "table; it reads no participant verdict and asks the table nothing about whether it is "
          "correct, only what it admits",
    ...
    Check("kernel contract against SPEC.md", [sys.executable, "scripts/sov_kernel.py", "drift"],
          ROOT,
          "re-reads SPEC.md bytes and derives the transition set and refusal codes from the "
          "governing document rather than trusting the authored table it is checking",
          ("SPEC.md", "contracts/kernel-transitions.json")),
    Check("kernel participant parity", [sys.executable, "scripts/sov_kernel.py", "parity"],
          ROOT,
          "compares each participant's declared refusal vocabulary against the kernel contract, "
          "so a participant cannot vouch for its own correspondence",
    ...
    Check("product ground and canon joins",
          ...
          "ground and a ground claim no promise carries, refuses a promise that is canonical "
          "only because something was built that way, and reads GROUND.md and CANON.md at check "
          "time so an identifier declared in a record and absent from the document that owns its "
          "wording fails here rather than drifting",
    ...
    Check("receipt event vocabulary",
          ...
          "parses each service's own modules and reads the event names its source passes to its "
          "journal, rather than asking the service or a table what it emits; an event must then "
          "be a capability the map declares or an entry the manifest states a reason for, and an "
          "excuse the service stopped emitting fails too",
    ...
    Check("specification traceability", [sys.executable, "scripts/sov_spec.py", "trace"], ROOT,
          "walks from SPEC.md requirements to the evidence records that claim them and refuses a "
          "standing whose predecessor standing is unreached",
          ("SPEC.md", "PRD.md")),
    Check("lessons loop", [sys.executable, "scripts/sov_lessons.py", "check"], ROOT,
          "parses LESSONS.md and grades each standing against the tree rather than against the "
          "page's own summary of itself, and reads what counts as EFFECTIVE out of the check "
          "table it is an entry in, so a lesson cannot assert a check that is not running. The "
          "drain count does not refuse: decisions/0029 declined that on the reasoning that "
          "failing on an eighth lesson makes capture costly exactly when capture matters, and "
          "this closes the half of that residual which taxes nobody",
    ...
    Check("phase progress floor", [sys.executable, "scripts/sov_phase_progress.py", "check"],
          ...
          "between them against the preserved Phase-I floor; it never reads a stored board or "
          "prior gate report. A fall refuses because it is attributable to the edit that caused "
          "it; a stall is printed and recorded as debt, on the wall-clock reasoning "
          "decisions/0081 settled (reports/2026-08-27-phase-i-retro.md, finding 1)",
    ...
    Check("semantic cold-start task", [sys.executable, "scripts/sov_witness.py", "semantic"],
          ROOT,
          "judges the custody round trip by digests the witness computes itself rather than by "
          "any value the participant reported",
    ...
    Check("participant against its baseline", [sys.executable, "scripts/sov_baseline.py"], ROOT,
          "runs the participant in a separate process and grades it through the frozen oracle, "
          "which never imports participant code; the participant does not report its own verdict",
    ...
    Check("ticket contract corpora", [sys.executable, "scripts/sov_ticket.py", "selfcheck"],
          ...
          "coordination surface it describes; the issue-body cases start from bytes a person "
          "could paste into GitHub, so a shape the schema admits but no ticket body can express "
          "fails here",
    ...
    Check("closure ownership", [sys.executable, "scripts/sov_closure.py", "selfcheck"], ROOT,
          ...
          "the participant that wrote them, and refuses a contract that declares a refusal no "
          "case proves fires or that leaves nothing reachable: every code must name an operation "
          "that clears it, and at least one the refused participant can take alone",
    ...
    Check("standing authority grants", [sys.executable, "scripts/sov_grant.py", "selfcheck"],
          ROOT,
          "grades a declared corpus of requests against grants the corpus itself carries, so the "
          "evaluator is never asked whether the issued registry is correct, only what it admits; "
          "it also validates every issued grant against the schema and refuses a corpus that "
          "leaves any reachable refusal code unproven",
    ...
    Check("node registry", [sys.executable, "scripts/sov_node.py", "validate"], ROOT,
          "reads the checked-in registry and the seat topology and grades one against the other; "
          "it contacts no peer and opens no socket, so the check cannot pass or fail because of "
          "what another node happened to be doing",
    ...
    Check("local model adapter",
          ...
          ROOT,
          "grades declared bindings and invocation records against a recorded runtime inventory "
          "rather than a live daemon, so the result cannot depend on whether a model server "
          "happens to be running on the checking machine",
    ...
    Check("Record Service independent witness",
          ...
          "performs the witness walk declared on issue #7 without importing the participant: the "
          "service is reached only as a subprocess through its CLI, every digest is recomputed "
          "from the chain rule the charter states, and the interrupt is staged against the store "
    ...
    Check("Kernel binding closure",
          ...
          "rebuilds cross-service binding facts from manifests, paradigms, and Kernel "
          "transitions; it does not ask service implementations whether their declarations are "
          "coherent",
    ...
    Check("Node Interface projection",
          ...
          "rebuilds from current source digests and compares the checked projection byte-for- "
          "byte; the projection cannot make itself reachable or observed",
    ...
    Check("documentation reader",
          ...
          "re-renders every published document from its bytes on disk and compares the page byte "
          "for byte, so a document that changed without a rebuild fails here rather than being "
          "shown under a receipt for an older version",
    ...
    Check("operation surface page",
          ...
          "manifest at the moment of the check and compares bytes, so a page edited by hand or "
          "left behind by a manifest change fails rather than misinforming a reader",
    ...
    Check("automation health",
          ...
          "reads the schedule declarations and the run ledger at the moment of the check and re- "
          "derives every reading, rather than believing the rendered page; the page is then "
          "compared byte for byte against that derivation so it cannot go stale silently, and "
          "where this machine holds no ledger the check says the run-history half is UNCHECKED "
          "and names the absent source instead of grading it green. An UNHEALTHY reading refuses "
          "here, which is the only alert Phase I admits",
    ...
    Check("repository tooling tests", [sys.executable, "scripts/run_tooling_tests.py"], ROOT,
          "the harness's own tests; independent of the repository content they check, but not of "
          "the harness itself; the runner partitions the complete discovered module population "
          "and fails if any shard fails",
          ("scripts/tests", "scripts/run_tooling_tests.py")),
)

#: Checks above whose only way to fail is that behaviour is wrong; every other
#: check keeps the main-gate default, per the reasoning the adding commit carries.
_DEV_GATE = frozenset({
    "repository hygiene", "status claim refusals fire", "owner queue",
    "bootstrap and locked evidence", "conformance oracle controls", "conformance oracle tests",
    "asset object cases", "kernel transition contract", "kernel participant parity",
    "service manifest contract", "receipt event vocabulary", "phase progress floor",
