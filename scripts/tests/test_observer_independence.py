...
class TheBasisIsRecordedRatherThanBlurred(unittest.TestCase):
    ...
    def test_a_request_carrying_a_resolved_observation_path_is_supplied_and_resolved(self):
        """The landing tool knows where the observation lives, so the relation is read
        rather than taken on trust. An earlier form reached the resolved verdict only when the
    ...
    def test_supplied_and_resolved_is_reachable_by_an_ordinarily_named_observer(self):
        """The defect this replaced: the verdict was unreachable in practice."""
    ...
    def test_a_path_the_caller_did_not_resolve_is_not_measured(self):
        ...
        An independent reading obtained the resolved verdict for `observation_path: "."` and for a
