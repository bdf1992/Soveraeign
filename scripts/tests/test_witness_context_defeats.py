...
UNREACHABLE = {
    "has no closing brace":
        "lexer.unreadable refuses an unbalanced `{` before any rule reads the function",
    "return array is unterminated":
        "lexer.unreadable refuses an unbalanced `[` before `blocks` reads the array",
    "no function named":
    ...
    "a regex literal opens and never closes":
        ...
        "line-ended check, which the corpus fires as D64",
}
...
def _refusal_texts(source: str) -> list[str]:
    ...
    A refusal here is a string handed to `Unreadable(...)` or appended to a violation
    list. Only the literal parts are returned, and only the ones long enough to identify a
    message: an f-string's interpolated values are runtime text and cannot be matched
    against a recorded violation.
    """
    out: list[str] = []
    for node in ast.walk(ast.parse(source)):
        if not isinstance(node, ast.Call):
            continue
        callee = node.func
        named = getattr(callee, "id", None) or getattr(callee, "attr", None)
        if named not in {"Unreadable", "append"}:
            continue
        for argument in node.args:
            out.extend(_literal_parts(argument))
    return [text for text in out if len(text) >= 25]
