...
class TheCorpusIsWellFormed(unittest.TestCase):
    def test_every_case_is_declared_completely(self):
        ...
        for case in cases:
            ...
            # Unique, so the mutation a case demonstrates is the one it names. `.replace(
            # ..., 1)` silently takes the first of several, and an independent reading found
            # two cases whose anchor matched more than once: each still fired its own
            # refusal, and neither was pinned to the construction it describes.
            source = live()[case["file"]]
            for key in ("replace", "also_replace"):
                patch = case.get(key)
                if patch:
                    self.assertEqual(
                        source.count(patch["old"]), 1,
                        f"{case['id']}: its {key} anchor matches "
                        f"{source.count(patch['old'])} places in {case['file']}, so which "
                        "mutation the case demonstrates is not determined")
...
UNREACHABLE = {
    "a prompt helper has no closing brace":
        "the body is balanced on the mask, so a `{` inside a string is not a brace and a "
        "genuinely unbalanced one is refused by lexer.unreadable before any rule reads it",
    "has no closing brace":
        "the same, one module over: render._body balances the mask through the one "
        "balancer in scan.py, and lexer.unreadable refuses a real imbalance first",
    "what this prompt delivers cannot be read":
        "every construction that reaches the rendering branch is refused one step earlier "
        "by the assembly branch, which resolves the same text and raises first. Kept as "
        "the fail-closed branch it is; a reading that reaches it defeats this entry",
    "a group opens and never closes in a prompt expression":
        "lexer.unreadable refuses an unbalanced `(` before any rule reads the prompt, "
        "which the corpus fires as D83. Kept as the fail-closed branch it is",
    "no function named":
    ...
    "unterminated string literal":
        "lexer.unreadable refuses an unclosed string before `_literals` reads it. An "
        "independent reading reached it another way, through a depth-zero regex holding a "
        "quote, which desynchronised `_literals` on JavaScript the lexer accepts; that is "
        "repaired at the cause - `_literals` now ends a regex by the mask's own rule - "
        "rather than by widening this entry",
    "unterminated comment":
        "lexer.unreadable refuses an unclosed block comment before `_literals` reads it",
    "cannot read expression at":
        "structurally unreachable rather than pre-empted, which is a different thing and is "
        "now recorded as the thing it is. An independent reading built every delimiter-"
        "opening slice it could - `, 'x'`, `)foo`, `] + 'a'`, `} 'a'`, `: 'a'` - and each "
        "read cleanly: the value scan always advances at least one character, because every "
        "character that reaches it is neither a quote nor a `+`. Kept as the fail-closed "
        "branch it is; a reading that reaches it defeats this entry",
}
...
#: find again.
REFUSAL_SOURCES = ("audit.py", "bindings.py", "calls.py", "dispatch.py", "escapes.py",
                   "frames.py", "lexer.py", "names.py", "render.py", "scan.py")
...
class EveryRefusalIsDemonstrated(unittest.TestCase):
    def test_no_refusal_ships_without_a_case_or_a_recorded_reason(self):
        ...
        self.assertEqual(sorted(stale), [],
                         ...
                         + "\n  ".join(sorted(stale)))
        # An entry a case does fire is an exemption that has stopped being one, and it
        # would go on exempting whatever it matched next. Two independent readings found
        # entries here claiming gates that did not exist; an entry that is simply wrong in
        # the other direction has to fail too, or the register only ever grows.
        exempt_but_fired = [reason for reason in UNREACHABLE
                            if any(reason in violation for violation in fired)]
        self.assertEqual(sorted(exempt_but_fired), [],
                         "UNREACHABLE exempts refusals a case now fires:\n  "
                         + "\n  ".join(sorted(exempt_but_fired)))
        undemonstrated = []
        for name in REFUSAL_SOURCES:
            module = ROOT / "scripts" / "sovprompts" / name
