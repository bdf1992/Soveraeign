...
Three limits, stated rather than implied, because each was found by an independent
...
class EveryWitnessDispatchIsFramed(unittest.TestCase):
    ...
    def test_the_scanner_has_not_desynchronised(self):
        """A second reading of the same files, by a method that cannot desynchronise.

        `calls.py` skips comments and regular expressions to avoid reading prose as code,
        and it cannot detect when that scan goes wrong. Two forms of it did: an apostrophe
        in a comment hid six dispatches, and a quote inside `.replace(/"/g, "'")` hid every
        dispatch below it in sov-loop.js. Both were silent, and both removed work from the
        check rather than adding it.

        An independent reading found the surviving case - a division the heuristic reads as
        a regex - and observed that what contains it is `lexer.unreadable` happening to use
        a different rule. Containment by two readers disagreeing is luck, and it ends
        quietly the day someone unifies them as a cleanup.

        Counting the substring `agentType` is not a parse and has no state, so it cannot
        desynchronise. Every dispatch declares one. When the two disagree, the scan is
        wrong and this says so rather than the count silently falling.
        """
        for path in sorted(WORKFLOWS.glob("*.js")):
            source = path.read_text(encoding="utf-8")
            self.assertEqual(
                source.count("agentType"), len(agent_calls(source)),
                f"{path.name}: the scanner and a plain substring count disagree about how "
                "many dispatches this file has, so the scan has desynchronised and "
                "dispatches are missing from every rule")
