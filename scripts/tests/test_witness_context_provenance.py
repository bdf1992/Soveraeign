...
Nothing here detects a desynchronised scan. `calls.py` skips comments and regular
expressions so that prose is not read as code, and it cannot tell when that goes wrong;
two forms of it went wrong silently, hiding six dispatches and then every dispatch below
one line. A substring-parity cross-check was written for this, and withdrawn: it failed a
legal file whose two dispatches share one options `const` - the very pattern the
classifier had just been repaired to read - because parity is a budget rather than an
invariant, and a budget is paid off by whoever notices what it measures. A proxy that
convicts correct code is worse than the gap it covers. The known remaining case is a
backtick string crossing lines, which both this reader and `lexer.unreadable` accept.
