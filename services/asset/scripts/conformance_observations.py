...
def main() -> int:
    with TemporaryDirectory() as tmp:
        ...
        try:
            ...
            proposal_id = service.propose(original["asset_id"], "model-1", {"description": "candidate"})
            proposal = service.db.execute("SELECT * FROM proposals WHERE id=?", (proposal_id,)).fetchone()
            ...
            run_id = service.request_derivative(original["asset_id"], original["version_id"], "Bdo")
            fence = service.claim(run_id, "worker-1")
            derived_version_id = service.report_derivative(run_id, "worker-1", fence, b'{"derived":true}')
            derived = service.db.execute("SELECT * FROM versions WHERE id=?", (derived_version_id,)).fetchone()
            derivation = json.loads(derived["derivation_json"])
            after = service.db.execute("SELECT digest FROM sources WHERE id=?", (original["source_id"],)).fetchone()[0]
            i2 = item("RUN-I2-REMEMBER", {
                ...
                "recording": {
                    ...
                    "reader_id": derivation.get("operation"),
                    "reader_version": None,
            ...
            i3 = item("RUN-I3-CROSS", {
                "human": {"transition_contract": "asset-service-python-api", "direct_write": False, "receipt_id": original["receipt_id"]},
                "model": {"transition_contract": None, "direct_write": False, "receipt_id": None},
                "crossing": {"source_id": None, "source_version": None, "projection_id": None, "omissions": None, "destination_id": None},
            })
            ...
            relation_id = service.db.execute("SELECT id FROM relationships WHERE proposal_id=?", (relation_proposal,)).fetchone()[0]
            retract_receipt_id = service.retract("relationship", relation_id, "Bdo", "superseded use")
            retract_receipt = service.db.execute("SELECT * FROM receipts WHERE id=?", (retract_receipt_id,)).fetchone()
            retract_payload = json.loads(retract_receipt["payload_json"])
            i4 = item("RUN-I4-RETRACT", {
                "original_record_present": service.db.execute("SELECT 1 FROM relationships WHERE id=?", (relation_id,)).fetchone() is not None,
                "counter_record_present": service.db.execute("SELECT 1 FROM retractions WHERE target_id=?", (relation_id,)).fetchone() is not None,
                "effect_class": "RECORD_LOCAL",
                "claims_world_rollback": False,
                "receipt": {"outcome": retract_receipt["outcome"], "target_receipt_id": retract_payload.get("target_receipt_id")},
                "reconstructed_prior_receipt_id": ratify_receipt,
            })

            refused_proposal = service.propose(original["asset_id"], "model-1", {"description": "judgement"})
            try:
            ...
            authority_receipt = service.db.execute(
                "SELECT id FROM receipts WHERE event='authority.check' AND subject_id=? ORDER BY created_at DESC LIMIT 1",
                (refused_proposal,),
            ).fetchone()
            ...
            i6 = item("RUN-I6-JUDGEMENT", {
                "pending_right": {"persistent": False, "visible": False, "outcome": None, "receipt_id": None},
