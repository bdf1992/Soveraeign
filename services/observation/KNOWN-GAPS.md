...
| **The perspective axis is a declaration too** | `RunRecord.profile_of` reads `payload.profile.digest` off a `LAUNCH` entry and resolves it against nothing. `decisions/0100` has a builder launch its own witness, so that entry's author is the executor. A third witness pass launched a second session of the executor with a digest of its choosing and was admitted as independent. `decisions/0104` Ruling 3 says perspective is the axis a declaration cannot remove, and its stated safeguard - "the launcher is not the observer" - names the wrong adversary: it stops the observer vouching for itself and leaves the executor vouching for the observer | A profile digest that resolves to bytes the executor did not author, or a refusal when the launcher is a version of an executor. Either changes what a `LAUNCH` entry is, which is above this boundary | `decisions/0104` judgement item 4; `reports/2026-09-08-independence-repairs.md`, F3 |
| An executor can erase an edge by writing a null | A `lease` written as `null` is a legitimate "no lease", so `HOLDS_RUN_LEASE` cannot fire; the same record with the real lease reads `DIRECT`. The unreadable-lease case refuses and the honestly-negative one cannot. Same shape as the row above and a smaller instance of it | Negative facts that something other than the executor attests | `reports/2026-09-08-independence-repairs.md`, F3 |
| An experienced observer is refused | A candidate the record shows moving any other subject reads `UNDETERMINED`, so an observer with prior work anywhere is unattestable. This is deliberate: pass 2 objected to it, pass 3 defeated the alternative by having the executor corroborate a decoy it had itself moved, and a subject nobody can verify is not a subject the walk may guess at. It fails closed, and it is still a cost | A subject corroborated by something the executor does not write | `reports/2026-09-08-independence-repairs.md`, F1 |
| Nothing resolves a citation in prose | `decisions/0104`, `CHARTER.md` and the pass 1 witness record cited `reports/2026-09-08-independence-repairs.md` for a day while it existed in no commit. `verify.py`'s citation check reads journal exports; no check resolves a repository path named in a governing document, so a decision record can cite a file that has never existed and every gate stays green | A check that resolves cited repository paths in covered prose | `reports/2026-09-08-independence-repairs.md`, R1 |
...
| Inference evidence is produced and barely read | Every inference carries `evidence_addresses` and `evidence_digests`. The admission gate checks that the addresses are entries this record carries and nothing checks the digests at all, so an inference can cite a real entry and a wrong digest for it | The digests re-derived at admission, or the field dropped as decoration. A field nothing grades is a claim nobody checks | `witness/independence-context-perspective.md`; pass 2 residual R1 |
| A complete record and a small one read alike | An inference records no journal head, watermark, or slice identity, and the entry slice is caller-supplied. `record_completeness: COMPLETE` therefore means "every edge was answerable", not "the record was whole", and no reader can tell the two apart | A recorded watermark the inference is read against, so narrowing the slice is visible | `witness/independence-context-perspective.md`; pass 2 residual R2 |
