...
| Receipt ownership | The service records its own receipts, one per attempt, in memory and, through `cli.py`, as one file each under its store | Either that, or a `terminal-receipt` in the Record Service journal. Four services own a private receipt type and nothing says how they relate to the journal's | `contracts/receipt.schema.json`; `services/record/contracts/service.json` |
| Durable state | `cli.py` keeps requests, declarations, inferences, observations, and receipts as one JSON file per record, named by id, under `--store DIR` (default `.local/observation`), and rebuilds an `ObservationService` from them on every invocation, so predicates declared in one process hold for an observation made in a later one (`store.py`). The in-process API alone still holds them in memory. `observation_id` names the run, the observer, and the addresses read, so a second `observe-run` over the same addresses refuses `OBSERVATION_RECORDED` (kernel `STALE_STATE`) and every committed observation is on disk and readable by id. The store is service-owned and not the journal: `observe-run` prints the `OBSERVATION` entry payload the observer passes to the Record Service's `append-entry`, and appends nothing itself | A projection over the journal or a service-owned store the Record Service can reconstruct. A file store per record is a default taken, enough to make an observation reproducible from the artifact and nothing more | `AGENTS.md` State and execution; `store.py` |
...
| The existing observer script | `scripts/witness_observe.py` still computes digests and predicates outside this boundary. `cli.py observe-run` now does the same work through the declared operation, with `declare-predicates --from-record` deriving the digest predicates from the run's `OUTPUT` entries, so the script no longer has a reason to exist that this service cannot serve | The same work behind `observe-run`, or a stated reason it belongs in scripts | `AGENTS.md` Directory boundaries |
| Settlement | `settle_run` has been performed once, by hand, by the controller of run `urn:soveraeign:run:thin-circuit-1` after `python scripts/sov_kernel.py check` returned `PERMITTED`; the receipt is in `reports/observations/2026-09-05-thin-circuit-1-journal.json`. `tests/test_kernel_parity.py` proves the kernel would accept this service's observation for `settle_run`; still no participant performs the transition as an operation | `settle_run` consuming a satisfactory observation and refusing `OBSERVATION_MISSING` without one | `SPEC.md` Transition contract |
...
| Two-binding proof | No binding drives this service; the tests call it in-process and, in `tests/test_cli.py`, as a subprocess of `cli.py`. A command line is a machine interface, not a human or model binding | A human binding and a model binding passing the same fixtures | PROD-I-3; `AI-NATIVE.md` check 7 |
| Independent observation of itself | Three witness passes by a participant that did not build it observed the thin slice through its declared surface (`witness/observation-service.md`, commit 3087714). One run, `urn:soveraeign:run:thin-circuit-1`, has been observed twice through `observe-run` by an observer this service inferred `INDEPENDENT`; both observations are in `reports/observations/2026-09-05-thin-circuit-1-journal.json`. That run was a thin circuit over this service's own gap R13, driven from a session scratchpad rather than `cli.py`; no run of any other service has been observed | An observation of a run, made through `observe-run`, by an observer this service inferred independent, reproducible from the artifact | C7; the recursion is real at the run level and resolved once, by hand |
